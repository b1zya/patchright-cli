...
Contents: storage state (save, restore, file format); cookies (list, filter by domain or path, get, set, delete, clear, multiple cookies); local storage; session storage. State files belong to the identity that produced them.
...
patchright-cli cookie-set remember_me token123 --expires=1893456000
```
