...
- Headless sessions present a real screen: headless Chrome reports 800x600 with no taskbar on any machine. With a display, the screen is the host's primary monitor (size, work area, scale), measured without a window, so a profile reports the same screen headed and headless; without one (a server, an SSH or service session) a common desktop of the platform stands in, with a note (`headless-screen`). The window fills the work area. Chrome's own scrollbars stay: Playwright's `--hide-scrollbars` would make them 0 px wide. `--window-size`, `--screen-info` through `--extra-arg`, `--device` and `--mobile` keep their own values.
...
- The headless user agent is a `Emulation.setUserAgentOverride` with metadata Playwright derives from the string: brands and full versions stay real, `platformVersion` becomes "10.0" on Windows and "10_15_7" on macOS instead of the build-derived value ("20.0.0" on a current Windows 11). No known vendor scores it; `selftest` reports the client-hint versions. Headed has the real value.
- A headless window keeps two small differences from a maximized headed one: `outerWidth - innerWidth` is 16 (a restored window's frame) instead of 0, and `outerHeight - innerHeight` is 8 px taller. The screen, the available screen and the outer window size match.
- A minimized headed window contradicts itself: focus emulation keeps it "visible" and focused while Chrome draws about one frame a second. A window covered by others or on another monitor stays consistent; do not minimize a session's window.
...
| `headless-screen` | INFO (note) | headless with no display to measure: a common desktop of the platform instead of 800x600 |
